            ...
            <div className="lead-layout">
              <div className="card full lead-list-card">
                ...
                {!filteredLeads.length ? (
                  <div className="empty">{leads.length ? "Nenhum com filtro" : "Sem leads"}</div>
                ) : (
                  <table className="lead-tbl">
                    ...
                    <tbody>
                      {filteredLeads.map((l) => (
                        <tr
                          key={l.id || l.phone}
                          className={"clickable" + (l.id && l.id === selectedLeadId ? " selected" : "")}
              ...
              <div className="card full lead-detail-card">
                <div className="lead-detail-head">
                  <div>
                    <h3>
                      {detailLead?.name || detailLead?.phone || (selectedLeadId ? "Lead" : "Selecione um lead")}
                    </h3>
                    <div className="sub" style={{ marginTop: 4 }}>
                      {leadDetail
                        ? `${leadDetail.message_count} msgs · ${fmtDateFull(detailLead?.updated_at)}`
                        : "Clique em uma linha"}
                    </div>
                  </div>
                  <div className="lead-detail-actions">
                    {selectedLeadId && (
                      <>
                        ...
                        >
                          {detailLead?.agent_paused ? "Retomar agente" : "Assumir conversa"}
                        </button>
                        <button
                          className="btn btn-ghost"
                          onClick={() => {
                    ...
                    {!leadDetail.messages.length ? (
                      ...
                      <div className="chat-scroll">
                        {leadDetail.messages.map((m, i) => {
                          ...
                          return (
                            <div key={`${m.created_at}-${i}`} className={"chat-row " + (isUser ? "user" : "assistant")}>
                              <div
                                className="chat-bubble"
                                style={!isUser && agentColor ? { borderColor: agentColor + "55" } : undefined}
                              >
                                {m.content || ""}
                              </div>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (!wsName.trim() || !wsSlug.trim()) {
                    setWsErr("Nome e slug obrigatórios.");
                    return;
                  }
                  try {
