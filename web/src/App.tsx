                        ...
                        >
                          ...
                          <td>
                            ...
                            {l.metadata?.agent_paused || l.metadata?.human_takeover ? (
                              <span className="tag" style={{ color: "var(--signal)", marginLeft: 4 }}>
                                ⏸
                              </span>
                            ) : null}
                          </td>
              ...
              <div className="card full lead-detail-card">
                <div className="lead-detail-head">
                  <div>
                    <div className="lead-title-row">
                      ...
                      {detailLead?.agent_paused && (
                        <span className="tag stage-badge" style={{ color: "var(--signal)" }}>
                          humano
                        </span>
                      )}
                    </div>
                  ...
                  <div className="lead-detail-actions">
                    {selectedLeadId && (
                      <>
                        ...
                        <button
                          className={"btn " + (detailLead?.agent_paused ? "btn-accent" : "btn-ghost")}
                          disabled={pauseBusy || leadDetailLoading}
                          onClick={() => void togglePause()}
                          title={detailLead?.agent_paused ? "Retomar agente IA" : "Pausar agente e assumir conversa"}
                        >
                          {pauseBusy
                            ? "…"
                            : detailLead?.agent_paused
                ...
                {leadDetail && !leadDetailLoading && detailLead && (
                  <>
                    ...
                    <div className="notes-panel">
                      ...
                      <div className="note-compose">
                        <textarea
                          ...
                          placeholder="Observação só para o time (não vai no WhatsApp)…"
                          value={noteText}
                        ...
                        >
                          {noteBusy ? "Salvando…" : "Salvar nota"}
                        </button>
                      </div>
                      {!(detailLead.notes && detailLead.notes.length) ? (
                        <div className="empty" style={{ padding: "12px 0" }}>
                          Nenhuma nota ainda
                        </div>
                      ) : (
                        <div className="notes-list">
                          {[...(detailLead.notes || [])].reverse().map((n, i) => (
                            <div className="note-item" key={`${n.at}-${i}`}>
                              <div className="note-body">{n.note}</div>
                              <div className="note-meta">
                                {n.author || "admin"} · {fmtDateFull(n.at)}
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    <div className="lead-events-panel">
                      <h2>Timeline do lead</h2>
                      {!(leadDetail.events && leadDetail.events.length) ? (
                        <div className="empty" style={{ padding: "12px 0" }}>
                          Sem eventos
                        </div>
                      ) : (
                        <div className="lead-events">
                          {leadDetail.events.map((ev, i) => {
                            ...
                            const summary =
                              ...
                              "";
                            return (
                              <div className="lead-ev" key={ev.id || `${ev.type}-${i}`}>
                                <div className="dot" />
                                <div>
                                  <div className="etype">{ev.type || ""}</div>
                                  {summary ? (
                                    <div className="ev-summary">{String(summary).slice(0, 160)}</div>
                                  ) : null}
                                  <div className="meta">{fmtDate(ev.created_at)}</div>
                                </div>
                              </div>
                            );
                          })}
