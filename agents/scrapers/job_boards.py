...
def _guess_company_from_url(url: str) -> str:
    ...
    # boards.greenhouse.io/acme → path
...
def scrape_job_url(url: str) -> list[dict[str, Any]]:
    """
    Extrai 1+ vagas de uma URL (página única ou board API).
    Greenhouse board: https://boards-api.greenhouse.io/v1/boards/{token}/jobs
    ...
    # Normaliza para APIs públicas quando possível
    api_url = url
    if "greenhouse.io" in host and "/jobs" not in path and "boards-api" not in host:
        # boards.greenhouse.io/{token} → API list
    ...
    if "json" in ct or text.lstrip().startswith(("{", "[")):
        ...
        if data is not None:
            ...
            if "lever" in api_url or "lever" in host:
                ...
                if jobs:
                    return jobs
            # genérico JSON com title/content
...
def scrape_board(
    ...
    *,
    ...
    """Lista vagas de um board e filtra por palavra no título (ex.: SDR)."""
